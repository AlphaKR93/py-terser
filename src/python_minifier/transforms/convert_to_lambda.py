...
from python_minifier.transforms.general_minifications import is_side_effect_free
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if node.body:
                    ...
                    if isinstance(first, ast.Expr):
                        val = first.value
                        if isinstance(val, (ast.Constant, ast.Str, ast.Bytes)):
                            if isinstance(getattr(val, 'value', getattr(val, 's', None)), (str, bytes)):
                                has_doc = True

                if not has_doc and len(node.body) == 1 and isinstance(node.body[0], ast.Return):
                    ret = node.body[0]
                    args_spec = node.args
                    defaults_safe = True
                    ...
                    if defaults_safe:
                        ret_val = ret.value if ret.value is not None else self.add_child(ast.Constant(value=None), parent=node, namespace=node.namespace)
                        new_lambda = self.add_child(ast.Lambda(args=node.args, body=ret_val), parent=parent, namespace=node.namespace)

                        wrapped_val = new_lambda
                        if node.decorator_list:
                            for dec in reversed(node.decorator_list):
                                wrapped_val = self.add_child(
                                    ast.Call(func=dec, args=[wrapped_val], keywords=[]),
                                    parent=parent,
                                    namespace=node.namespace
                                )

                        new_target = self.add_child(ast.Name(id=node.name, ctx=ast.Store()), parent=parent, namespace=node.namespace)
                        new_assign = self.add_child(ast.Assign(targets=[new_target], value=wrapped_val), parent=parent, namespace=node.namespace)
