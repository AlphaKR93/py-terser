...
from terser.printer.token_printer import Delimiter, TokenPrinter
...
class ExpressionPrinter(object):
    ...
    def visit_Str(self, node):
        if hasattr(node, 'raw_representation'):
            self.printer.raw_string(node.raw_representation)
        else:
    ...
    def visit_JoinedStr(self, node):
        ...
        self.printer.fstring(str(terser.printer.strings.f_string.OuterFString(node, pep701=pep701)))

    def visit_TemplateStr(self, node):
        assert isinstance(node, ast.TemplateStr)

        import terser.printer.strings.t_string

        self.printer.tstring(str(terser.printer.strings.t_string.TString(node)))
