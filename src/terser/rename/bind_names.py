...
from terser.transforms.suite_transformer import NodeVisitor
...
class NameBinder(NodeVisitor):
    ...
    def visit_arg(self, node):
        ...
        def contains_annotated(n):
            if isinstance(n, ast.Name):
                return n.id == 'Annotated'
            if isinstance(n, ast.Attribute):
                return n.attr == 'Annotated'
            for child in ast.iter_child_nodes(n):
                if contains_annotated(child):
                    return True
            return False

        if node.annotation and contains_annotated(node.annotation):
            binding.disallow_rename()
